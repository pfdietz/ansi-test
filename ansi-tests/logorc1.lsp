...
(deftest logorc1.error.1
  (loop for x in *mini-universe*
	unless (or (integerp x)
		   (eq (eval `(classify-error (logorc1 ',x 0))) 'type-error))
	collect x)
  nil)
...
(deftest logorc1.error.2
  (loop for x in *mini-universe*
	unless (or (integerp x)
		   (eq (eval `(classify-error (logorc1 0 ',x))) 'type-error))
	collect x)
  nil)

(deftest logorc1.error.3
  (classify-error (logorc1))
  program-error)

(deftest logorc1.error.4
  (classify-error (logorc1 0))
  program-error)

(deftest logorc1.error.5
  (classify-error (logorc1 1 2 3))
  program-error)
